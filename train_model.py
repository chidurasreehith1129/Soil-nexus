...
def generate_master_data():
    ...
    for crop_code in range(27):
        for _ in range(1000):
            month = np.random.randint(1, 13)
            state = 1
            price_min, price_max = crop_price_ranges[crop_code]
            price = np.random.uniform(price_min, price_max)
            data.append([month, crop_code, state, price])

    df = pd.DataFrame(data, columns=['target_harvest_month', 'crop_code', 'state_code', 'modal_price'])
...
def train_model(data):
    ...
    X = data[['crop_code', 'target_harvest_month', 'state_code']]
    y = data['modal_price']
