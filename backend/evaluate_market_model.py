...
try:
    ...
    print(" -> Success! Historical dataset and ML model loaded successfully.")
...
X = df[['target_harvest_month', 'crop_code', 'state_code']]
y = df['modal_price']
...
print(f" * Root Mean Squared Error (RMSE): Rs. {rmse:.2f} per quintal")
...
while True:
    ...
    try:
        ...
        # Structure the simulated question for the model (2D Array)
        test_input = np.array([[user_month, user_crop_code, user_state_code]])
        
        # Get the prediction!
        predicted_price = float(model.predict(test_input)[0])
        ...
        print(f" -> Predicted Wholesale Price: Rs. {predicted_price:.2f} per quintal")
